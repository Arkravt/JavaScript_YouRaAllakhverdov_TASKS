...
// 3.
console.log('******************************* TASK 3 *******************************');
const arrKeys = [...events.keys()];
const matchВuration = arrKeys[arrKeys.length - 1];
const average = matchВuration / arrKeys.length;
console.log(`On average, an event happened every ${average} minutes`);


// 4.
console.log('******************************* TASK 4 *******************************');
for (let [time, eventName] of events) {
    let half = time <= 45 ? 'FIRST HALF' : 'SECOND HALF';
    console.log(`[${half}] ${time}: ${eventName}`);
}
